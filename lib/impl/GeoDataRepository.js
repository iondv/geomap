...
function GeoDataRepository(options) {
  ...
  this.bindCoord = function (item, type, coords) {
    if (!options.dataSource) {
      return Promise.reject(new Error('Не настроен источник данных словаря координат.'));
    }
  ...
  this._getLayerData = function (area, query, opts) {
    return new Promise(function (resolve, reject) {
      if (typeof query.getRestUrl === 'function') {
        ...
        // TODO учитывать вложенные уровни данных - ВОЗМОЖНО ФУНКЦИОНАЛ НЕ НУЖЕН
        ...
        if (typeof query.getLocationAttribute === 'function') {
          ...
          if (!options.dataSource) {
            return reject(new Error('Не настроен источник данных словаря координат.'));
          }
          let locationFilter = {
            [F.AND]: [
          ...
          reject(new Error('Некорректный объект описания данных слоя.'));
        }
      }
    });
  };
